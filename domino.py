...
def combinations_two(start, end, contain_self=False, repeat=1):
  for r in range(0, repeat):
    for i in range(start, end):
      j_start = i if contain_self else i + 1
...
BACK_TRI_LEFT = Color(0x88, 8, 8)
BACK_TRI_RIGHT = Color(55, 55, 55)
BACK_FONT_COLOR = Color(0, 0, 0)
BACK_LINE_WIDTH = 1
...
def draw_box(ctx: cairo.Context, xi: int, yi: int):
  ...
  ctx.arc(MARGIN_WIDTH + BOX_RADIUS + BOX_EXTRA_MARGIN + CARD_WIDTH*xi, yinv(MARGIN_HEIGHT +
          CARD_HEIGHT*(yi + 1) - BOX_RADIUS - BOX_EXTRA_Y), BOX_RADIUS, 3*pi/2, pi/2)
...
if __name__ == '__main__':
  ...
  for left, right in combinations_two(0, 7, contain_self=True, repeat=2):
    if (xi == 0 and yi == 0):
      if svg:
  ...
  if not svg:
    ...
    combined.write("Domino-Cards.pdf")
